...
public class WindowAliases extends Window implements IListBoxItemClickListener<Alias>, IWindowClosedListener<WindowEditOrCreateAlias>
{
    ...
    public void init()
    {
        ...
        desc.elements = pack.getContentRegistry("AliasManager").getContentList();
        desc.sorted = true;
        desc.canSelect = true;
        listBox = new ListBox<Alias>(desc, this);
        addControl(listBox);
    ...
    protected void controlClicked(Control c, int mouseX, int mouseY, int button)
    {
        ...
        else if (c == btnDelete)
        {
            ...
            listBox.updateElements(pack.getContentRegistry(Alias.class).getContentList());
            btnDelete.setEnabled(false);
            btnEdit.setEnabled(false);
        }
        else
